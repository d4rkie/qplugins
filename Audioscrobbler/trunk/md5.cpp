...
void md5(unsigned char *outbuffer, /* 16 bytes */
                const unsigned char *input)
{
  MD5_CTX ctx;
  MD5_Init(&ctx);
...
// 32 byte ascii version
// strDest is not zero terminated !
void md5_32(char* strDest, /* 32 bytes */
                const unsigned char *input)
{
	unsigned char strTemp[16] = {0};
	md5(strTemp, input);
	md5_convert_to_ascii(strDest, strTemp);
}
...
void md5_convert_to_ascii(char* strDest, const unsigned char md5Digest[16])
{
	for (int i = 0; i < 16; i++)
	{
		unsigned char n = md5Digest[i];
		strDest[i*2]   = "0123456789abcdef"[(n & 0xf0) >> 4];
		strDest[i*2+1] = "0123456789abcdef"[n & 0x0f];
	}
}