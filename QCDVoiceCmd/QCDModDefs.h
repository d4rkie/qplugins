...
{									//*** below returns numeric info (*buffer not used)
	...
	opGetNextIndex = 12,			// get index of next track to play (0 based), param = index start index. -1 for after current
	...
	opSetPlaylist = 1006,			// add files to or reset playlist with new files (buffer = file list (null term), param1 = (string ptr)originating path (can be NULL), param2 = clear playlist flag
